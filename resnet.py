...
#           beta controls sharpness (annealed during training)
# ============================================================
class Act8LevelTanhGrad(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x, levels, beta):
        # Compute transition midpoints between the 8 levels (7 transitions)
        # e.g. for levels [-1, -0.71, ..., +1], transitions are the midpoints
        transitions = (levels[:-1] + levels[1:]) / 2.0
        ctx.save_for_backward(x)
        ctx.transitions = transitions
        ctx.beta = beta
        # Forward: still hard argmin snap — UNCHANGED
    ...
    def backward(ctx, grad_out):
        ...
        # Gradient of smooth stacked tanh staircase
        # Each transition tᵢ contributes a bell curve:
        #   beta * (1 - tanh²(beta * (x - tᵢ)))
        # This peaks at x = tᵢ and falls to zero away from it
        # Sum of all 7 bell curves = meaningful gradient signal
